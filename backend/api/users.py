...
DEFAULT_USERS = [
    {"id": "u0", "username": "admin", "display_name": "管理员", "role": "超级管理员", "department": "管理层", "status": "正常", "last_login": "-", "password": "admin123"},
...
@router.post("")
async def create_user(user: UserCreate, current_user: dict = Depends(require_role("超级管理员"))):
    """Create a new user."""
    users = _load_users()
    ...
    _save_users(users)
    add_log(current_user["username"], "用户管理", f"创建用户 {user.username} ({user.display_name})")
    return {"status": "success", "user": {k: v for k, v in new_user.items() if k != "password"}}


@router.put("/{user_id}")
async def update_user(user_id: str, update: UserUpdate, current_user: dict = Depends(require_role("超级管理员"))):
    """Update user info."""
    users = _load_users()
    for u in users:
        if u["id"] == user_id:
            ...
            _save_users(users)
            add_log(current_user["username"], "用户管理", f"更新用户 {u['username']}: {', '.join(update_data.keys())}")
            return {"status": "success", "user": {k: v for k, v in u.items() if k != "password"}}
    raise HTTPException(status_code=404, detail="用户不存在")


@router.delete("/{user_id}")
async def delete_user(user_id: str, current_user: dict = Depends(require_role("超级管理员"))):
    """Delete a user."""
    users = _load_users()
    for i, u in enumerate(users):
        if u["id"] == user_id:
            deleted = users.pop(i)
            _save_users(users)
            add_log(current_user["username"], "用户管理", f"删除用户 {deleted['username']}")
